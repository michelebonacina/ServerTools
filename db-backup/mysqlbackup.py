                ...
                if result != 0:
                    ...
                    if log_size > 0:
                        ...
                        if (backup_dir_copy == True):
                            # copy dump file to destination dir
                            sys_command = backup_dir_copy_cmd.replace('[dump_file]', export_dir + os.path.sep + dump_name)
                            ...
                            sys_command = backup_dir_copy_cmd.replace('[dump_file]', export_dir + os.path.sep + log_name)
                            result = os.system(sys_command)
...
if len(detail_messages) > 0:
    ...
    with smtplib.SMTP_SSL(mail_server, mail_server_port, context=context) as server:
        ...
        message = []
        message.append('Subject: [' + socket.gethostname() + '] Backup dei database - Esito: ' + ('ERROR!!!' if error else 'OK'))
